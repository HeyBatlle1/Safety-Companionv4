...
"""
    ...
    def _generate_executive_decision(self, report: Dict[str, Any]) -> str:
        """Generate GO/NO-GO decision with reasoning (V1 style + LLM narrative)"""
...
"""
        ...
        # Key insight from LLM (if available)
        key_insight = exec_summary.get('keyInsight', '')
        if key_insight:
            section += f"### 🎯 Key Insight\n> {key_insight}\n\n"
        ...
        # Override note (if baseline was overridden)
        if go_no_go.get('wasOverridden') and go_no_go.get('overrideReason'):
            section += f"### ⚠️ Baseline Override\n_{go_no_go.get('overrideReason')}_\n\n"

        # Contradictions resolved (if any)
        contradictions = exec_summary.get('contradictionsResolved', '')
        if contradictions:
            section += f"### Cross-Agent Analysis\n{contradictions}\n\n"
...
"""
    ...
    def _generate_footer(self, report: Dict[str, Any]) -> str:
        ...
        synthesizer = metadata.get('synthesizedBy', 'Agent4-LLM-v1')
...
| Generated By | Safety Companion V3.1 Multi-Agent System |
| Agents | Agent 1 (Validator), Agent 2 (Risk Assessor), Agent 3 (Predictor), Agent 4 (LLM Synthesizer) |
| Synthesis Engine | {synthesizer} |
